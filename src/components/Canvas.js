...
const Canvas = forwardRef(({ ballot }, ref) => {
    ...
    if (!ballot1 && !stamp1) {
      loadImagePromise(ballot1Image)
        .then((image) => {
          ...
          if (ballotType !== 1) editor.canvas.add(ballot1Instance);
        ...
        .then((stampImage) => {
          ...
          if (ballotType !== 1) editor.canvas.add(stamp1Instance);
    ...
    if (!ballot2 && !stamp2) {
      loadImagePromise(ballot2Image)
        .then((image) => {
          ...
          if (ballotType !== 0) editor.canvas.add(ballot2Instance);
        ...
        .then((stampImage) => {
          ...
          if (ballotType !== 0) editor.canvas.add(stamp2Instance);
  ...
  }, [fabricRef, editor?.canvas, ballotType, topBallot]);
